...
export default function NewAdvertiserAdPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isLoading) {
      alert('로그인 정보 확인 중입니다. 잠시만 기다려주세요.');
      return;
    }
    if (!user) {
      alert('로그인 후 이용해 주세요.');
      ...
      <div className="bg-white shadow-sm border-b">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            ...
            <div className="flex items-center space-x-4">
              <Link href="/advertiser/ads" className="text-gray-600 hover:text-gray-700 text-sm font-medium">
                광고 목록으로
              </Link>
            </div>
          </div>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="flex justify-end space-x-4">
            ...
            <button
              type="submit"
              disabled={isSubmitting || isLoading || !user || (adType === 'regional' && selectedRegions.length === 0)}
              className="px-6 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
            >
              {isSubmitting ? '등록 중...' : isLoading ? '로그인 확인 중...' : '광고 등록'}
            </button>
          </div>
        </form>
      </div>
